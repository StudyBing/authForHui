package com.bing.water.common.config;


import com.bing.water.common.utils.DESUtils;
...
/**
 * Created by xuguobing on 2016/10/19 0019.
 */
...
//                String encryptedString = matcher.group(1);    //获得加密值
                String encryptedString = propertyValue.replaceAll("Encrypted:\\{", "");
                encryptedString = encryptedString.substring(0, encryptedString.lastIndexOf("}"));
                String decryptedPropValue = null;  //调用AES进行解密，SEC_KEY与属性名联合做密钥更安全
                try {
                    decryptedPropValue = DESUtils.decrypt(encryptedString).trim();
                    ...
                    logger.error("Decrypt " + propertyName + "=" + propertyValue + " error!", e);
                }
            }
        }
                    ...
                    if (isNotBlank(line)) {   //是否为空行
                        ...
                        if (!line.startsWith("#")) {//如果是非注释行
                            ...
                            if (lineParts != null && lineParts.length >= 2) {
                                String key = lineParts[0].trim();       // 属性名
                                ...
                                if (key != null && value != null) {
                                    ...
                                    if (encryptedProps.contains(key)) { //发现是加密属性
                                        ...
                                        if (!matcher.matches()) { //如果是非加密格式，则`进行加密
                                            value = ENCRYPTED_PREFIX + DESUtils.encrypt(value) + ENCRYPTED_SUFFIX;   //进行加密，SEC_KEY与属性名联合做密钥更安全
    ...
    public static void main(String[] args) throws Exception {
        String es = DESUtils.encrypt("jdbc:mysql://10.1.64.80:9034/healthcloud?useUnicode=true&characterEncoding=utf-8");
