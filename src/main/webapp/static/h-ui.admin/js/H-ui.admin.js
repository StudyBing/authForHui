...
function Hui_update_iframe(name,url) {
	var topWindow = $(window.parent.document);
	var tab = topWindow.find("#min_title_list>li");
	var iframe = topWindow.find('#iframe_box .show_iframe');
	var has = false;
	$.each(tab,function(){
		if($.trim($(this).find("span").text()) == name) {
			has = true;
			$("#min_title_list>li.active", window.parent.document).remove();
			$("#iframe_box>div:visible", window.parent.document).remove();

			var i = tab.index($(this));
			tab.eq(i).addClass("active");
			iframe.eq(i).show();
			var curIframe = iframe.eq(i).find("iframe");
			$(curIframe).attr("src",$(curIframe).attr("src"));
			return;
		}
	});
	if (!has){
		var span = $("#min_title_list>li.active>span", window.parent.document);
		span.attr("data-href",url);
		span.text(name);
		$("#iframe_box>div:visible>iframe", window.parent.document).attr("src",url);
	}
